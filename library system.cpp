...
struct books {
	...
	void print_books_with_prefix(string pre) {
		...
		if (!found)
			cout << "there is no books with prefix " << pre << flush;
		cout << '\n' << flush;
	}
	void sort_by_name() {
	...
	void return_book(string name_of_user, int id_of_book) {
		Books[id_of_book].users_of_book.erase(
				Books[id_of_book].users_of_book.find(name_of_user));
		Books[id_of_book].quantity++;
	}
...
struct users {
	...
	void return_book(string name_of_user, string name_of_book) {
		...
		User[user_index].borrowed_books.erase(
				User[user_index].borrowed_books.find(name_of_book));
...
void bingo() {
	...
	while (true) {
		...
		if (c == 0) {
			...
			//add user
			cout << "Enter the name of user " << flush;
			string s;
			cin >> s;
			cout << "Enter the id of user " << flush;
			int id;
			cin >> id;
			u1.add_user(s, id);
		} else if (c == 7) {
			...
			string s1;
			cin >> s1;
			cout << "Enter the name of the user " << flush;
			string s2;
			cin >> s2;
